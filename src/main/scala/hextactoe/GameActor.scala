...
trait GameActor {
  ...
  def meshName: String
  def meshFile: String
  def meshSetup: typings.babylonjs.BABYLON.AbstractMesh => Unit
  def turnMsg: String
  ...
  def createActorMarker(state: GameState, target: BABYLON.Vector3, onFinished: ScaleTweenParameters => Unit) = {
    BABYLON.SceneLoader.ImportMesh(state.activeActor.meshName, meshesUrl, state.activeActor.meshFile, state.scene, (newMeshes, _, _, _, _, _, _) => { 
        val parent = new BABYLON.Mesh("", state.scene)
        meshSetup(newMeshes(0))
        parent.addChild(newMeshes(0)) 
        parent.position = target 
        parent.scaling = BABYLON.Vector3(0,0,0)
        val t =RotationTween.rotateAround(Duration(4, SECONDS), parent, BABYLON.Vector3(0,0,1), state.tweenMgr)
        t.start
        ScaleTween.scaleTo(Duration(300, MILLISECONDS), parent, BABYLON.Vector3(1,1,1), Some(onFinished)).runOn(state.tweenMgr)
    }, {}, {}, {})
  }
}
...
object Player extends GameActor {
  val id = 0
  val colour = BABYLON.Color3(0, 0.75, 1)
  val meshName = "satelliteDish_detailed"
  val meshFile = "satelliteDish.glb"
  val meshSetup = m => { m.rotate(BABYLON.Vector3(1,0,0), math.Pi/6); m.scaling.scaleInPlace(0.12) }
...
object Opponent extends GameActor {
  val id = 1
  val colour = BABYLON.Color3(1, 1, 0)
  val meshName = "turret"
  val meshFile = "turret.glb"
  val meshSetup = _.rotate(BABYLON.Vector3(1,0,0), math.Pi/4)
