...
final case class LinearMoveTweenParameters(d: Duration, val mesh: BABYLON.Mesh, val dest: BABYLON.Vector3, val origin: BABYLON.Vector3) 
  extends TweenParameters[LinearMoveTweenParameters](d, v => mesh.position = MoveTween.v3lerp(v, origin, dest), LoopType.PingPong(3), EaseType.Sigmoid, Duration.Zero, None)
